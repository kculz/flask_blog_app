...
def blog():
    ...
    posts = Post.query.order_by(Post.date_post.desc()).paginate(page=page, per_page=3)
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and bcrypt.check_password_hash(user.password, form.password.data):
            ...
            flash('Login Un-successful! Please Check E-mail & Password!', 'danger ')
...
def new_post():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(post)
        db.session.commit()
        flash('Your Post Have Been Created Successfully', 'primary')
        return redirect(url_for('blog'))
...
def delete_post(post_id):
    ...
    db.session.commit()
    flash('Post Was Deleted Successfully', 'primary')
    return  redirect(url_for('blog'))
